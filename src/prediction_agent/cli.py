...
from .paper_store import attribution, paper_review, record_report, settle_pending, summary as paper_summary
...
def main() -> None:
    ...
    attr = sub.add_parser("paper-attribution", help="group settled paper bets by lineup, EV tier, direction, and sport")
    attr.add_argument("--paper-db", default="data/daily/paper.db")
    attr.add_argument("--since", default=None)
    ...
    elif args.command == "paper-attribution":
        print(json.dumps(attribution(args.paper_db, since=args.since), ensure_ascii=False, indent=2))
    else:
