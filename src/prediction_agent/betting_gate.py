...
It never touches a wallet, exchange SDK, or private key. ``研究建议`` is a
...
def can_place_real_bet(match: dict[str, Any], db_path: str | Path | None = None) -> tuple[bool, str]:
    """Return whether the research output may be labelled ``研究建议``.
    ...
    return True, f"虚拟已完成{virtual_count}场且ROI={virtual_roi:.1%}，升级为研究建议"
...
def bet_status(match: dict[str, Any], db_path: str | Path | None = None) -> str:
    ...
    if virtual_ok:
        ...
        return "研究建议" if real_ok else "虚拟研究"
    return "跳过"
